...
- [globalpocket/mcp-routing-gateway](https://github.com/globalpocket/mcp-routing-gateway) - LLM에 제시되는 툴을 안전하게 관리(필터링 및 가상화)하기 위한 스테이트리스 로컬 MCP 라우팅 게이트웨이 및 Pure Proxy.
...
- [globalpocket/mcp-mlx-launcher](https://github.com/globalpocket/mcp-mlx-launcher) - Apple Silicon 환경에서 로컬 LLM을 운용하는 `mlx-lm` 인스턴스의 모델 다운로드부터 관리, 시작, 종료까지 지원하는 MCP 서버.
