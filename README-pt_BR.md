...
- [memcell-ai/cli](https://github.com/memcell-ai/cli) [![memcell-ai/cli MCP server](https://glama.ai/mcp/servers/memcell-ai/cli/badges/score.svg)](https://glama.ai/mcp/servers/memcell-ai/cli) 📇 🏠 🍎 🪟 🐧 - Memória viva para agentes de codificação IA (Claude Code, Cursor, Copilot, Codex). Memória persistente entre ferramentas, recuperação de decisões e calibração de resultados via MCP e hooks. `npm install -g memcell`
