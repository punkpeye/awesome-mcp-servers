...
- [atbridge-ai/atbridge](https://github.com/atbridge-ai/atbridge) [![atbridge-ai/atbridge MCP server](https://glama.ai/mcp/servers/atbridge-ai/atbridge/badges/score.svg)](https://glama.ai/mcp/servers/atbridge-ai/atbridge) 🎖️ 🏠 🍎 🪟 🐧 - Servidor MCP local para Thunderbird — e-mails, calendários, contatos, notas e tarefas em todas as caixas de correio (Gmail, Outlook, IMAP). Privado por design: fala com o seu Thunderbird em execução, sem nada trafegando pela nuvem; leitura aberta, escrita controlada. Funciona com qualquer cliente MCP (Claude Desktop, Cursor, …). Instale em https://atbridge.ai/pt/download e execute `atbridge mcp setup`.
