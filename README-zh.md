...
- [atbridge-ai/atbridge](https://github.com/atbridge-ai/atbridge) 🎖️ 🏠 🍎 🪟 🐧 - 面向 Thunderbird 的本地 MCP 服务器 —— 在所有邮箱（Gmail、Outlook、IMAP）中处理邮件、日历、联系人、笔记和任务。隐私优先设计：与正在运行的 Thunderbird 本地通信，不经过云端；读取默认开放，写入受控。兼容任意 MCP 客户端（Claude Desktop、Cursor 等）。从 https://atbridge.ai/download 安装，然后运行 `atbridge mcp setup`。
