...
- [team-telnyx/telnyx-mcp-server](https://github.com/team-telnyx/telnyx-mcp-server) 🎖️ 🐍 ☁️ 🏠 - AI駆動通信アプリケーションを構築するための公式MCPサーバー。音声アシスタントの作成、SMSキャンペーンの送信、電話番号の管理、リアルタイムメッセージングの統合を企業レベルの信頼性で実現。リモート [streamable-http](https://api.telnyx.com/v2/mcp) と [sse](https://api.telnyx.com/mcp/sse) サーバーを含む。
