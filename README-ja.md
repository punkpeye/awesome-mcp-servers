...
- [gapup-team/gapup-mcp](https://github.com/getgapup/gapup-mcp-public) [![smithery badge](https://smithery.ai/badge/gapup-team/gapup-mcp)](https://smithery.ai/servers/gapup-team/gapup-mcp) 📇 ☁️ - 100以上のエージェント課金型Cスイート専門知識を1つのMCPに集約 — 競合インテリジェンス（EDGAR＋Yahoo＋Wayback）、SECファイリングデコーダー、8リスト制裁スクリーナー（OFAC＋EU＋UK＋UN＋SECO＋SEMA＋DFAT）、KYC、ペンテスト範囲、GRADE評価済み臨床エビデンス（PubMed＋ClinicalTrials.gov＋OpenFDA）、EUファースト不動産（DVF Cerema＋Géorisques）、NAICSクラシファイヤー、センチメントニュース（GDELT）、研究論文Q&A（OpenAlex 4500万件）。183ツール。Base＋Optimismでx402 USDC/EURCによるペイパーコール。月100回無料。
