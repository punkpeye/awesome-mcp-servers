...
- [opendatalab/MinerU-Ecosystem](https://github.com/opendatalab/MinerU-Ecosystem/tree/main/mcp) 🎖️ 🐍 🏠 ☁️ - 公式 MinerU ドキュメント解析 MCP（PyPI: [mineru-open-mcp](https://pypi.org/project/mineru-open-mcp/)）。[MinerU](https://mineru.net) API により PDF、doc/docx、ppt/pptx、画像、スプレッドシートを Markdown に変換。API キー不要の無料 Flash モード（ファイルあたり約 20 ページ）。任意で `MINERU_API_TOKEN` を設定すると上限拡張。
