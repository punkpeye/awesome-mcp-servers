...
- [s60yucca/mnemos](https://github.com/s60yucca/mnemos) 🏎️ 🏠 🍎 🪟 🐧 - AIコーディングエージェント向けの永続メモリエンジン。単一のGoバイナリ、ゼロランタイム依存、MCPネイティブ。埋め込みSQLiteを使用してセッション間でメモリを保存・検索・重複排除し、ハイブリッドFTS+セマンティック検索、メモリ減衰、関係グラフ、トークン予算コンテキストアセンブリをサポート。
