...
- [ailenshen/apple-notes-mcp](https://github.com/ailenshen/apple-notes-mcp) [![apple-notes-mcp MCP server](https://glama.ai/mcp/servers/ailenshen/apple-notes-mcp/badges/score.svg)](https://glama.ai/mcp/servers/ailenshen/apple-notes-mcp) 📇 🏠 🍎 - Apple Notesの読み書きとMarkdownの双方向変換。SQLite直接クエリによる高速なリスト・検索、AppleScript＋ネイティブインポートによる完全なCRUD操作。stdioとStreamable HTTPトランスポートに対応。
