...
- [peg/rampart](https://github.com/peg/rampart) 🏎️ 🏠 🍎 🐧 - Policy engine that wraps MCP servers with security enforcement. Evaluates every tool call against YAML policies, blocks dangerous operations before they reach the server, and logs everything to a hash-chained audit trail. Supports auto-policy generation via `rampart mcp scan`.
