...
- [preflight-dev/preflight](https://github.com/preflight-dev/preflight) 📇 🏠 🍎 🪟 🐧 - 24-tool MCP server for Claude Code that catches ambiguous prompts before they waste tokens in wrong→fix cycles. Features prompt scoring with 12-category scorecards, semantic session history search via LanceDB, cross-service contract awareness, correction pattern learning, and cost estimation.
