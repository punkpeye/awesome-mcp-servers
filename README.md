...
- [AetherCore-Dev/ag402](https://github.com/AetherCore-Dev/ag402) [glama](https://glama.ai/mcp/servers/AetherCore-Dev/ag402) 🐍 ☁️ 🏠 🍎 🪟 🐧 - Payment layer for MCP servers implementing the Coinbase x402 protocol. Monetize any MCP server or HTTP API with one command (`ag402 serve`), zero code changes. Solana USDC, $0.00025/tx, ~0.5s settlement, 1ms prepaid mode. 588+ tests, 4 internal security reviews, 6-layer budget protection. Non-custodial, zero telemetry. Native support for Claude Code, Cursor, OpenClaw, LangChain, and CrewAI. Already powering live mainnet payments.
- [AetherCore-Dev/token-rugcheck](https://github.com/AetherCore-Dev/token-rugcheck) [glama](https://glama.ai/mcp/servers/AetherCore-Dev/token-rugcheck) 🐍 ☁️ - Solana token safety audit for AI agents. Three-layer report: machine verdict (risk score 0-100), LLM-friendly analysis (red/green flags), and raw on-chain evidence. Data from RugCheck, DexScreener, and GoPlus with concurrent fetch and graceful degradation. Live on Solana mainnet, $0.02 USDC per audit via x402 micropayments.
