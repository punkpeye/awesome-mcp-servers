...
- [TerminalGravity/preflight](https://github.com/TerminalGravity/preflight) 📇 🏠 🍎 🪟 🐧 - 24-tool MCP server for Claude Code that catches vague prompts before they waste tokens on wrong→fix cycles. Features 12-category prompt scorecards, correction pattern learning, cross-service contract awareness, session history search with LanceDB vectors, and cost estimation.
