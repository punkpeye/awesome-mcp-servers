...
- [KGT24k/mcp-tool-search](https://github.com/KGT24k/mcp-tool-search) 📇 🏠 🍎 🪟 🐧 - MCP proxy that reduces context token usage by 85-96% through lazy tool loading. Replaces all backend tool schemas with 4 lightweight proxy tools (search, inspect, call, list) with fuzzy search via Levenshtein distance.
