...
- [docxi-org/zobr-script](https://github.com/docxi-org/zobr-script) [glama](https://glama.ai/mcp/servers/docxi-org/zobr-script) 📇 ☁️ - ZS (Zobr Script) — cognitive scripting language for structured reasoning with LLMs. MCP server provides language spec, interpreter prompt, example scripts, and full PEG parser validation. Models execute `.zobr` scripts step by step using 12 built-in cognitive operations (survey, assert, doubt, contrast, synthesize, etc.).
