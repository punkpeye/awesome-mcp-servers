...
- [0xbrainkid/satp-mcp](https://github.com/0xbrainkid/satp-mcp) [![0xbrainkid/agentfolio-mcp-server MCP server](https://glama.ai/mcp/servers/0xbrainkid/agentfolio-mcp-server/badges/score.svg)](https://glama.ai/mcp/servers/0xbrainkid/agentfolio-mcp-server) 📇 ☁️ 🏠 🍎 🪟 🐧 - AI agent trust verification via SATP (Solana Agent Trust Protocol). 8 tools to check trust scores, verify on-chain identity, browse 200+ registered agents, and query attestation history from AgentFolio. Works with Claude Code, Cursor, and Claude Desktop. Free, open source.
