...
- [FHIRfly-io/fhirfly-mcp-server](https://github.com/FHIRfly-io/fhirfly-mcp-server) [![fhirfly-mcp-server MCP server](https://glama.ai/mcp/servers/FHIRfly-io/fhirfly-mcp-server/badges/score.svg)](https://glama.ai/mcp/servers/FHIRfly-io/fhirfly-mcp-server) 🎖️ 📇 ☁️ - Healthcare reference data for AI agents: 50+ tools across 21+ US & UK code systems — providers (NPI), drugs (NDC, RxNorm, FDA labels, drug interactions), diagnoses (ICD-10, SNOMED), labs (LOINC), procedures (HCPCS, MS-DRG), claims edits, and HCC risk adjustment. Free tier, no credit card. `npx -y @fhirfly-io/mcp-server`
