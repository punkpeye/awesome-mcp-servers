...
- [poweroutlet2/openground](https://github.com/poweroutlet2/openground) 🐍 🏠 - On-device RAG system that ingests official documentation from git repos, sitemaps, or local directories into a local vector database. Exposes documentation via MCP with hybrid vector+BM25 search using LanceDB and local embeddings. Includes specialized Claude Code agent for documentation queries. Open source and local alternative to Context7.
