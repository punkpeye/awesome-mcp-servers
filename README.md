...
- [tipdotmd/tip-md-x402-mcp-server](https://github.com/tipdotmd/tip-md-x402-mcp-server) 📇 ☁️ - MCP server for cryptocurrency tipping through AI interfaces using x402 payment protocol and CDP Wallet. Provides tools for wallet type retrieval, tipping with USDC, manual tipping for ETH and SOL (not support by x402 yet), checking balances, exporting wallets, and withdrawing funds. Enables AI assistants to facilitate direct crypto tips to registered developers without leaving the chat interface.
