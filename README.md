...
- [kael-bit/engram-rs](https://github.com/kael-bit/engram-rs) 🦀 🏠 🍎 🪟 🐧 - Three-layer memory engine (Buffer→Working→Core) with automatic lifecycle management. Memories promote, decay, and merge based on importance and reinforcement — not everything is worth keeping forever. Hybrid semantic + keyword recall, per-component LLM routing, single binary.
