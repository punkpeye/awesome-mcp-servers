...
- [jtalk22/slack-mcp-server](https://github.com/jtalk22/slack-mcp-server) 📇 ☁️ 🏠 🍎 🪟 🐧 - Full Slack workspace access via session mirroring—DMs, channels, search, threads—without OAuth or admin approval. Production-grade: atomic writes, mutex locks, auto-refresh, rate limiting with backoff. 11 MCP tools, Magic Link auth, interactive demo, and Web UI for claude.ai.
