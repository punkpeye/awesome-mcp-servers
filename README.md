...
- [pouriya/mcpd](https://github.com/pouriya/mcpd) MCPD: an MCP server that turns any executable script into an MCP tool. It discovers scripts in a directory, extracts metadata via --help, and exposes them to MCP clients like Claude, Cursor, or any LLM-based agent. It includes a web dashboard for testing and supports HTTP/HTTPS, Authentication, CAPTCHA, and MCP Resources.
