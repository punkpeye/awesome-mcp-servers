...
- [EtanHey/brainlayer](https://github.com/EtanHey/brainlayer) 🐍 🏠 🍎 🐧 - Persistent memory layer for AI coding agents. Indexes Claude Code conversations into a local SQLite-vec database with hybrid search (semantic + FTS5). 14 MCP tools: search, think, recall, store, file timeline, regression detection, session history, and more. 268K+ chunks indexed, local enrichment via Ollama. 266 tests.
