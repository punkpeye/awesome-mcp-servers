...
- [0xbrainkid/agentfolio-mcp-server](https://github.com/0xbrainkid/agentfolio-mcp-server) [glama](https://glama.ai/mcp/servers/0xbrainkid/agentfolio-mcp-server) 📇 ☁️ 🏠 🍎 🪟 🐧 - MCP server for [AgentFolio](https://agentfolio.bot) — agent identity, trust scores, marketplace jobs, reviews, and endorsements. 8 tools for profile management, job discovery, and on-chain SATP verification. Install via `npx agentfolio-mcp-server`.
