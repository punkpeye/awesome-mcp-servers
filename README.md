...
- [AutropicAI MCP Directory](https://tryautropic.com) 📇 ☁️ - Searchable MCP server marketplace with 200+ indexed servers and hosted tool APIs. Includes semantic search and an MCP server for discovering other MCP servers. - [askbudi/roundtable](https://github.com/askbudi/roundtable) 📇 ☁️ 🏠 🍎 🪟 🐧 - Meta-MCP server that unifies multiple AI coding assistants (Codex, Claude Code, Cursor, Gemini) through intelligent auto-discovery and standardized MCP interface, providing zero-configuration access to the entire AI coding ecosystem.
