...
- [lordbasilaiassistant-sudo/base-flash-arb-mcp](https://github.com/lordbasilaiassistant-sudo/base-flash-arb-mcp) 📇 ☁️ - Arbitrage detection across DEXes on Base. 7 tools: scan pairs, detect price gaps, estimate profit, simulate flash loan routes, and monitor spreads in real time.
- [lordbasilaiassistant-sudo/base-gasless-deploy-mcp](https://github.com/lordbasilaiassistant-sudo/base-gasless-deploy-mcp) 📇 ☁️ - Gasless ERC-20 token deployment on Base using CDP Paymaster. 5 tools: deploy tokens, verify contracts, check deploy status, estimate gas savings, and list deployments.
- [lordbasilaiassistant-sudo/base-multi-wallet-mcp](https://github.com/lordbasilaiassistant-sudo/base-multi-wallet-mcp) 📇 ☁️ - Coordinated multi-wallet trading on Base. 8 tools: create wallets, distribute funds, execute synchronized buys/sells, rebalance portfolios, and aggregate balances.
- [lordbasilaiassistant-sudo/base-price-oracle-mcp](https://github.com/lordbasilaiassistant-sudo/base-price-oracle-mcp) 📇 ☁️ - On-chain price feeds from Base DEX pools with no external API dependencies. 7 tools: get spot prices, TWAP, liquidity depth, price impact estimates, and historical OHLCV from pool reserves.
...
- [lordbasilaiassistant-sudo/base-security-scanner-mcp](https://github.com/lordbasilaiassistant-sudo/base-security-scanner-mcp) 📇 ☁️ - Smart contract security scanner for Base. 8 tools: detect honeypots, rug pull patterns, hidden mints, proxy dangers, bytecode analysis, permission checks, and generate audit reports.
