...
- [rsdouglas/janee](https://github.com/rsdouglas/janee) 📇 🏠 🍎 🪟 🐧 - MCP server for secrets management. Store API keys encrypted locally, agents request access via MCP `execute` tool. Features: per-service capabilities, TTL-based access, auto-approval/manual review, full audit logs. Your keys stay on your machine, agents never see them. Install: `npm install -g @true-and-useful/janee`
