...
- [preflight-dev/preflight](https://github.com/preflight-dev/preflight) 📇 🏠 - 24-tool MCP server for Claude Code that catches vague prompts before they cost you 2-3x in wrong→fix cycles. Features 12-category prompt scorecards, semantic session history search with LanceDB vectors, correction pattern learning, cross-service contract awareness, and token cost estimation.
