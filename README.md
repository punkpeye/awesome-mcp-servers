...
- [RelayPlane/mcp-server](https://github.com/RelayPlane/proxy/tree/main/packages/mcp-server) 📇 ☁️ 🏠 - Budget-aware LLM proxy MCP server for AI coding agents. Route requests across LLM providers with per-project cost tracking and spend caps. Tools: check_budget, route_request, get_cost_report, set_budget_alert. Compatible with Claude Code, Cursor, and Windsurf. Available on npm as [@relayplane/mcp-server](https://www.npmjs.com/package/@relayplane/mcp-server).
