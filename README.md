...
- [hurttlocker/cortex](https://github.com/hurttlocker/cortex) 🏎️ 🏠 🍎 🐧 🪟 - Import-first, observable memory layer for AI agents. Hybrid search (BM25 + semantic), Ebbinghaus confidence decay, fact extraction, lifecycle policies, knowledge graph, and 9 connectors (GitHub, Gmail, Obsidian, etc.). Single binary, SQLite storage, `brew install`. Runs as MCP server via `cortex mcp` (stdio or HTTP+SSE).
