...
- [yedanyagamiai-cmd/openclaw-mcp-servers](https://github.com/yedanyagamiai-cmd/openclaw-mcp-servers) 📇 ☁️ 🍎 🪟 🐧 - 9 remote MCP servers on Cloudflare Workers for AI agents: JSON toolkit, regex engine, color palette generator, timestamp converter, prompt enhancer, AI market intelligence, fortune/horoscope, content publisher (Markdown to HTML), and AI coding tool comparison. Free tier + Pro API keys ($9). Zero-install, works with any MCP client.
