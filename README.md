...
- [ihwooMil/mcp-memory-server](https://github.com/ihwooMil/mcp-memory-server) 🐍 🏠 🍎 🪟 🐧 - Intelligent memory with RL-powered save/skip/retrieve policy, GraphRAG hybrid retrieval (ChromaDB + knowledge graph), multi-resolution text compression, forgetting pipeline, and sleep cycle maintenance. 12 MCP tools, multilingual (ko/en).
