...
- [dev55acc-ai/paid-mcp-metering](https://github.com/dev55acc-ai/paid-mcp-metering) 📇 ☁️ - Metered MCP tools with per-call x402 v2 USDC billing on Base: free discovery via MCP (`initialize`, `tools/list`, `ping`); paid execution behind HTTP 402 with `PAYMENT-SIGNATURE`. Tools: `web_search` ($0.001/call), `deep_research` ($0.01/call). Upstreams Exa (primary) + Tavily (fallback). Publishes to the official MCP Registry as `io.github.dev55acc-ai/paid-mcp-metering`. ▶ See also: [Glama listing](https://glama.ai/mcp/servers/dev55acc-ai/paid-mcp-metering)
- [mainlayer/paid-mcp-server](https://github.com/mainlayer/paid-mcp-server) 📇 ☁️ - Production-ready MCP server template where every tool call passes through a Mainlayer payment gate: agents pay via Mainlayer (payment infrastructure for AI agents) and retry; tool runs only after entitlement check passes. Includes 6 tools (4 paid, 2 free): `analyze_code` ($0.05/analysis), `generate_image_prompt` ($0.02/call), `summarize_url` ($0.03/call), plus free tools. Pricing guidelines: $0.001 simple lookups, $0.005 search/data, $0.01 AI inference. Publishes to the official MCP Registry as `io.github.mainlayer/paid-mcp-server`. ▶ See also: [Glama listing](https://glama.ai/mcp/servers/mainlayer/paid-mcp-server)
