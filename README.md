...
- [Ataraxy-Labs/sem](https://github.com/Ataraxy-Labs/sem) 🦀 🏠 🍎 🪟 🐧 - Entity-level code intelligence over a deterministic cross-file graph: impact ("what breaks if I change this"), token-budgeted context (a function plus its callers and callees), semantic diff, and structural search across 32 languages via tree-sitter. Deterministic, so no hallucinated edges or missed callers.
