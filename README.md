...
- [AI Applyd MCP](https://aiapplyd.com/mcps) 📇 ☁️ - Remote job-search MCP server for Claude, Cursor and ChatGPT: ATS resume scoring, job-description analysis, job search, interview prep, resume optimization and translation, cover letters, PDF resume builds, and auto-apply that submits on the employer's hiring system and confirms the submission registered before reporting. Streamable HTTP at `https://mcp.aiapplyd.com/mcp` with OAuth 2.1 and dynamic client registration, so there is no install and no config file.
