...
- [Lykhoyda/ask-gemini-mcp](https://github.com/Lykhoyda/ask-gemini-mcp)[![Lykhoyda/ask-gemini-mcp MCP server](https://glama.ai/mcp/servers/Lykhoyda/ask-gemini-mcp/badges/score.svg)](https://glama.ai/mcp/servers/Lykhoyda/ask-gemini-mcp) 📇 ☁️ 🍎 🪟 🐧 - Bridge Claude with Google Gemini CLI for AI-to-AI collaboration. Get second opinions on code, debate architecture plans, and review changes using Gemini's 1M+ token context window. Supports multi-turn sessions, sandbox mode, structured edits, and automatic Pro→Flash quota fallback.
