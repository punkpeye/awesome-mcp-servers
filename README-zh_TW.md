...
- [ailenshen/apple-notes-mcp](https://github.com/ailenshen/apple-notes-mcp) [![apple-notes-mcp MCP server](https://glama.ai/mcp/servers/ailenshen/apple-notes-mcp/badges/score.svg)](https://glama.ai/mcp/servers/ailenshen/apple-notes-mcp) 📇 🏠 🍎 - 讀寫 Apple Notes 並支援 Markdown 雙向轉換。透過 SQLite 直連實現快速列表/搜尋，透過 AppleScript + 原生匯入實現完整 CRUD。支援 stdio 和 Streamable HTTP 傳輸。
